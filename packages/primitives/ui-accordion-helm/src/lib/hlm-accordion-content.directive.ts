...
export class HlmAccordionContentDirective implements OnInit {
  ...
  public readonly cssHeight = computed(() =>
    this.height() === '-1' ? 'auto' : this.height()
  );
    ...
    if (isPlatformBrowser(this._platformId)) {
      this._changes = new MutationObserver((mutations: MutationRecord[]) => {
        mutations.forEach((mutation: MutationRecord) => {
          ...
          const state = (mutation.target as any).attributes.getNamedItem(
    ...
    effect(
      () => {
        ...
        Promise.resolve().then(() => {
          this.height.set(
            isOpen
              ? getComputedStyle(this._element).getPropertyValue(
                  '--brn-collapsible-content-height'
                )
              : '0px'
          );
        });
  ...
  @HostBinding('style') get customStyles(): Partial<CSSStyleDeclaration> {
    return {
